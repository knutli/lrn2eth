...
import "./4-priceConverter.sol";

contract FundMe {
    using PriceConverter for uint256;
    uint256 public minimumUSD = 5 * 1e18; //1 * 10 ** 18
    ...
    function howManyFunders() public view returns(uint) {
        return funders.length;
    }

    function sumOfFunds() public view returns(uint) {
    ...
    function fund() public payable {
        ...
        require(msg.value.getConversionRate() >= minimumUSD, "not enuf vespene gas"); // 1e18 == 1 * 10 ** 18 == 1 000 000 000 000 000 000
